...
def _get_presence_intervals(df: pd.DataFrame, side: Side, presence_duration_threshold_seconds=60) -> Tuple[
    ...
    for timestamp, row in df.iterrows():
        ...
        # to require. (BUG-05, 2026-07-09 incident night: cap_data.py's
        ...
        # the same fixture. Keeping the OR here rather than switching to
        # cap-only or AND, since piezo alone still misses still/breathing-
        # only occupancy and one incident night is not enough data to trust
