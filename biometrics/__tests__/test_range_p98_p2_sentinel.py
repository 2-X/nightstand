...
percentile calc. Two real incidents on the live pod:

  - 2026-07-09: a raw value of 2_148_008_185 (~2^31, an int32-overflow-style
    ...
    an empty bed.
  - 2026-07-10: both the left AND right piezo channels independently
    computed a range of exactly 16_777_215 (2^24 - 1) at the same one-second
    tick (see free-sleep-stream.log [presence-debug] line at 12:00:54 UTC).
...
# The two real values from the incident report.
REAL_GARBAGE_SENTINEL = 2_148_008_185   # 2026-07-09, ~2^31 overflow-style garbage
REAL_LEGITIMATE_MAX = 16_777_215        # 2026-07-10, 2^24 - 1, within legitimate range
...
class TestRangeP98P2SentinelRejection(unittest.TestCase):
    ...
    def test_garbage_sentinel_sample_does_not_blow_up_range(self):
        ...
        # 2026-07-09 incident. Masked, the range should stay close to the
    ...
    def test_legitimate_16m_value_is_not_rejected(self):
        # Half the window at the real 2026-07-10 legitimate max. This is
