"""Tests for capacitance-sensor presence detection (BUG-05).
...
never crossed detect_presence_cap's occupancy_threshold: replaying the
2026-07-09 incident-night RAW fixture (both sides occupied ~8 hours,
...
fixture) restored cap presence to ~90-99% of confirmed-occupied samples on
both sides, with 0% false positives across those same empty stretches.

These tests use small synthetic DataFrames (the real fixture is a local,
...
def _make_cap_df(values_by_sensor, freq_seconds=1, start='2026-07-09 04:00:00'):
...
class TestCreateCapBaselineFloor(unittest.TestCase):
    def test_default_floor_is_1_not_5(self):
        # BUG-05 regression: the shipped default used to be 5, which was
    ...
    def test_floor_only_engages_below_the_real_std(self):
        ...
        # fixture's measured empty-window std of 0.4-0.85): real std should
...
class TestDetectPresenceCapFloorBug(unittest.TestCase):
    """Reproduces the incident-night shape at Pod-5 scale: a small, real
    occupied-vs-empty delta (mean shifts ~8-14 units, matching the fixture's
    measured left_out/right_out deltas) against a baseline whose real std is
    tiny (~0.04, matching the fixture's measured calibration-window std).
    """

    def setUp(self):
        self.baseline_mean = {'left_out': 11.6, 'left_cen': 9.8, 'left_in': 15.5}
        # Occupied-bed values elevated similarly to the fixture's measured
    ...
    def _baseline_with_floor(self, min_std):
        ...
        # fixture's identified 5-minute baseline period (true std ~0.03-0.05).
        empty_values = {
