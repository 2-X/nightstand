...
Move to a named preset defined in `8sleep/basePresets.ts` (e.g. `flat`, `sleep`, `relax`, `read`).
...
The server merges the incoming side into its in-memory state (there is currently no WebSocket push for presence; clients poll).
...
| `service-health` | A check in `/api/serverStatus` flips between `healthy` and `failed` | Partial server-status patch, only the fields that changed |
