...
import type { AlarmSchedule, DailySchedule, Schedules } from './schedulesSchema.js';

const legacyAlarm = (overrides: Partial<AlarmSchedule> = {}): AlarmSchedule => ({
  time: '09:00',
  enabled: true,
...
const day = (overrides: Partial<DailySchedule> = {}): DailySchedule => ({
  temperatures: {},
...
const schedules = (leftMut: (s: ReturnType<typeof emptySide>) => void = () => {}): Schedules => {
  const left = emptySide();
  leftMut(left);
  return { left, right: emptySide() } as unknown as Schedules;
};
...
describe('migrateLegacyAlarms', () => {
  ...
  it('drops a legacy alarm with an invalid time instead of throwing', () => {
    ...
    const s = schedules((left) => {
      left.monday = day({ alarms: [{ ...legacyAlarm({ enabled: true }), time: '25:99' } as AlarmSchedule] });
    });
    const out = migrateLegacyAlarms(s);
