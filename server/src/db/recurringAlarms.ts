...
import { RecurringAlarmsDb } from './schedulesSchema.js';
import { migrateLegacyAlarms } from './recurringAlarmsMigration.js';
import schedulesDB from './schedules.js';
import config from '../config.js';
import logger from '../logger.js';

// On-disk shape adds the migration sentinel alongside the validated data.
type PersistedShape = RecurringAlarmsDb & { _migrated?: boolean };

const defaultData: PersistedShape = { left: [], right: [], _migrated: false };

const file = new JSONFile<PersistedShape>(`${config.lowDbFolder}recurringAlarmsDB.json`);
const recurringAlarmsDB = new Low<PersistedShape>(file, defaultData);
