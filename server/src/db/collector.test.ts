...
// Capture what the collector's flush writes, without a real DB. createMany
// records the rows per model; $transaction just resolves the ops.
type Captured = { bed: any[]; hub: any[]; events: any[]; audit: any[] };
const captured: Captured = { bed: [], hub: [], events: [], audit: [] };
let failNextTransaction = false;

const makeCreateMany = (bucket: keyof Captured) => async ({ data }: { data: any[] }) => {
...
let onDeviceStatus: typeof import('./collector.js')['onDeviceStatus'];
let recordEvent: typeof import('./collector.js')['recordEvent'];
let recordConfigAudit: typeof import('./collector.js')['recordConfigAudit'];
let flush: typeof import('./collector.js')['flush'];

before(async () => {
  ({ onDeviceStatus, recordEvent, recordConfigAudit, flush } = await import('./collector.js'));
});

const makeSide = (over: Partial<SideStatus> = {}): SideStatus => ({
...
const makeStatus = (over: Partial<DeviceStatus> = {}): DeviceStatus => ({
  left: makeSide(),
  right: makeSide(),
  waterLevel: 'true',
  isPriming: false,
