...
// Same fallback the runtime uses: prefer the alarms[] array, else the single
// legacy alarm object. Disabled entries are filtered out downstream (see
// normalize) so only enabled alarms migrate.
type LegacyAlarm = {
  time?: unknown;
...
function legacyAlarmsForDay(day: unknown): LegacyAlarm[] {
  if (!day || typeof day !== 'object') return [];
  const d = day as { alarms?: unknown; alarm?: unknown };
  if (Array.isArray(d.alarms) && d.alarms.length > 0) return d.alarms as LegacyAlarm[];
  if (d.alarm && typeof d.alarm === 'object') return [d.alarm as LegacyAlarm];
  return [];
}

function isValidPattern(p: unknown): p is 'double' | 'rise' {
...
// behaviour.
function normalize(alarm: LegacyAlarm): {
  time: string;
  intensity: number;
  pattern: 'double' | 'rise';
  duration: number;
  ...
  if (alarm.enabled !== true) return null;
  if (typeof alarm.time !== 'string' || !TIME_RE.test(alarm.time)) return null;
...
// of the key.
function groupKey(a: ReturnType<typeof normalize> & object): string {
  return `${a.time}|${a.intensity}|${a.pattern}|${a.duration}`;
}

function recurrenceForDays(days: number[]): RecurringAlarm['recurrence'] {
  const set = new Set(days);
  if (set.size === 7) return { kind: 'daily' };
  const isWeekdays = set.size === 5 && [1, 2, 3, 4, 5].every((d) => set.has(d));
  if (isWeekdays) return { kind: 'weekdays' };
  const isWeekends = set.size === 2 && set.has(0) && set.has(6);
  if (isWeekends) return { kind: 'weekends' };
  return { kind: 'customDays', days: [...set].sort((x, y) => x - y) };
}

function migrateSide(sideSchedule: unknown, side: Side): RecurringAlarm[] {
  // Map groupKey -> { alarm fields, set of day indexes it appeared on }.
  const groups = new Map<
    string,
    { fields: NonNullable<ReturnType<typeof normalize>>; days: Set<number> }
  >();

  DAY_ORDER.forEach((dayName, dayIndex) => {
    const day = (sideSchedule as Record<string, unknown>)?.[dayName];
 ...
 */
export function migrateLegacyAlarms(schedules: Schedules): RecurringAlarmsDb {
  return {
