...
export function parseDfOutput(output: string): DfResult | null {
  ...
  if ([totalKb, usedKb, availableKb].some((n) => Number.isNaN(n))) return null;

  return { totalKb, usedKb, availableKb };
}

export function parseDuOutput(output: string): number {
