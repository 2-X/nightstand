...
router.get('/temperature', async (req: Request, res: Response) => {
  ...
  const bedWhere: Prisma.bed_state_samplesWhereInput = {};
  if (side) bedWhere.side = side;
  const bedTimestamp: Prisma.IntFilter = {};
  if (startTime) bedTimestamp.gte = moment(startTime).unix();
  if (endTime) bedTimestamp.lte = moment(endTime).unix();
  if (startTime || endTime) bedWhere.timestamp = bedTimestamp;
  ...
  const hubTimestamp: Prisma.IntFilter = {};
  if (startTime) hubTimestamp.gte = moment(startTime).unix();
  if (endTime) hubTimestamp.lte = moment(endTime).unix();
