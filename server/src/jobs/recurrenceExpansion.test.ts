...
import type { RecurringAlarm } from '../db/schedulesSchema.js';

const TZ = 'America/New_York';

// Helper: local wall-clock instant in TZ.
const at = (iso: string, tz = TZ) => moment.tz(iso, tz).valueOf();

const alarm = (overrides: Partial<RecurringAlarm> = {}): RecurringAlarm => ({
  id: 'a1',
  time: '07:00',
...
function assertAllAtWallTime(occ: { epochMs: number }[], hhmm: string, tz = TZ) {
  for (const o of occ) {
...
describe('expandAlarmOccurrences', () => {
  ...
  it('customDays: only the listed weekday indexes', () => {
    ...
    const occ = expandAlarmOccurrences(
      alarm({ recurrence: { kind: 'customDays', days: [1, 4] } }),
      TZ,
      from,
      to,
    );
  ...
  it('everyNDays: fires on the anchor and every n-th day after, not before', () => {
    ...
    const occ = expandAlarmOccurrences(
      alarm({ recurrence: { kind: 'everyNDays', n: 3, anchorDate: '2026-03-01' } }),
      TZ,
      from,
      to,
    );
  ...
  it('everyNDays with n=1 is equivalent to daily from the anchor', () => {
    ...
    const occ = expandAlarmOccurrences(
      alarm({ recurrence: { kind: 'everyNDays', n: 1, anchorDate: '2026-03-01' } }),
      TZ,
      from,
      to,
    );
