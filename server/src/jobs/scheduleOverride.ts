...
export const markManualTempChange = async (side: Side): Promise<void> => {
  ...
  if (hoursUntil > OVERRIDE_WINDOW_HOURS) {
    logger.debug(
      `[manual temp] ${side}: next schedule at ${next.format()} is ${hoursUntil.toFixed(1)}h away (>${OVERRIDE_WINDOW_HOURS}h), no override.`,
    );
    return;
  }
  ...
  logger.info(
    `[manual temp] ${side}: schedule paused until ${expiresAt} (next change was at ${next.format()}, ${hoursUntil.toFixed(1)}h away).`,
  );
};
