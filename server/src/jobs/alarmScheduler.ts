...
export const executeAlarm = async (
  { vibrationIntensity, duration, vibrationPattern, side, force=false, refireUntilDismissed=false }: AlarmJob,
) => {
...
export function scheduleDeadlineRefires(args: DeadlineRefireArgs): void {
  const { side, firedAt, vibrationIntensity, duration, vibrationPattern } = args;
  let refireCount = 0;

  const dismissedSinceFire = async (): Promise<boolean> => {
    await memoryDB.read();
    ...
    const timer = setTimeout(() => {
      void (async () => {
        try {
          ...
          await settingsDB.read();
          if (settingsDB.data[side].awayMode) return;
          const franken = await connectFranken();
          const status = await franken.getDeviceStatus();
          if (!status[side].isOn) return;
...
function scheduleSmartWakeSession(side: Side, alarm: RecurringAlarm, occurrenceMs: number) {
  ...
  if (fireAtMs >= occurrenceMs) return; // window already fully elapsed
  ...
  try {
    ...
    logger.debug(
      `Scheduled smart-wake session ${side}/${alarm.id} start at ${new Date(fireAtMs).toISOString()} `
