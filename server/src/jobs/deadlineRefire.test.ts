...
const alarmCommands: Array<{ command: string; arg: string }> = [];
mock.module(new URL('../8sleep/deviceApi.js', import.meta.url).href, {
  namedExports: {
    executeFunction: async (command: string, arg = 'empty') => { alarmCommands.push({ command, arg }); },
  },
});
...
const refireEvents: Array<Record<string, unknown>> = [];
mock.module(new URL('../db/collector.js', import.meta.url).href, {
  namedExports: {
    recordEvent: (type: string, opts: { payload?: Record<string, unknown> }) => {
      if (type === 'refire') refireEvents.push(opts.payload ?? {});
    },
    recordConfigAudit: () => {},
  },
});
...
let scheduleDeadlineRefires: typeof import('./alarmScheduler.js')['scheduleDeadlineRefires'];
