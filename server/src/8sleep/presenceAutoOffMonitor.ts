...
const lastSeenOnAt: Record<Side, number | null> = { left: null, right: null };
const prevIsOn: Record<Side, boolean | null> = { left: null, right: null };
...
async function tick(): Promise<void> {
  ...
  if (clockStepped) {
    ...
    for (const side of ['left', 'right'] as const) {
      if (lastSeenOnAt[side] !== null) lastSeenOnAt[side] = now;
    }
    return;
  }
  ...
  for (const side of ['left', 'right'] as const) {
    ...
    if (presence[side].present) continue;
    ...
    const refs: number[] = [];
    if (lastSeenOnAt[side] !== null) refs.push(lastSeenOnAt[side]!);
    if (lastPresenceAt !== null) refs.push(lastPresenceAt);
...
export function startPresenceAutoOff(): void {
  if (timer) return;
  logger.info(
    `presenceAutoOff: starting (timeout ${PRESENCE_AUTO_OFF_MS / 60_000}min, check every ${CHECK_INTERVAL_MS / 1_000}s)`,
  );
  timer = setInterval(() => { void tick(); }, CHECK_INTERVAL_MS);
}

export function stopPresenceAutoOff(): void {
  if (timer) {
    clearInterval(timer);
    timer = null;
  }
