...
function encodeUint(n: number): Buffer {
  if (n <= 0x17) return Buffer.from([n]);
  if (n <= 0xff) return Buffer.from([0x18, n]);
  if (n <= 0xffff) {
    const b = Buffer.alloc(3);
    b[0] = 0x19; b.writeUInt16BE(n, 1); return b;
  }
  const b = Buffer.alloc(5);
  b[0] = 0x1a; b.writeUInt32BE(n, 1); return b;
}

function encodeByteStringHeader(len: number): Buffer {
  if (len <= 0x17) return Buffer.from([0x40 | len]);
  if (len <= 0xff) return Buffer.from([0x58, len]);
  if (len <= 0xffff) {
    const b = Buffer.alloc(3);
    b[0] = 0x59; b.writeUInt16BE(len, 1); return b;
  }
  const b = Buffer.alloc(5);
  b[0] = 0x5a; b.writeUInt32BE(len, 1); return b;
}
