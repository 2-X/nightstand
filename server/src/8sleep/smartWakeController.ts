...
import {
  ...
  SmartWakeSnapshot,
  SmartWakeDeps,
  SmartWakeJournalEvent,
  SMART_WAKE_TICK_MS,
  SMART_WAKE_STALE_VITALS_MS,
} from './smartWake.js';
...
type ActiveEntry = {
  session: SmartWakeSession;
  timer: ReturnType<typeof setInterval> | null;
  inFlight: boolean;
  // Set when a dismissal is observed on this side after the session started.
  dismissed: boolean;
};

const active: Partial<Record<Side, ActiveEntry>> = {};

// --- concrete deps ---------------------------------------------------------

async function readSnapshot(side: Side): Promise<SmartWakeSnapshot> {
...
async function pulse(side: Side, intensity: number, durationS: number): Promise<void> {
...
function makeDeps(side: Side, entry: ActiveEntry): SmartWakeDeps {
  return {
    now: () => Date.now(),
    ...
    armVibe: () => armVibe(),
    journal: (event: SmartWakeJournalEvent, payload) => {
 ...
 */
export async function startSmartWakeSession(params: SmartWakeParams): Promise<void> {
  try {
    ...
    const entry: ActiveEntry = {
      session: null as unknown as SmartWakeSession,
      timer: null,
      inFlight: false,
      dismissed: false,
    };
 ...
 */
export function stopSmartWakeSession(side: Side, reason = 'stopped'): void {
 ...
 */
export function notifySmartWakeDismissed(side: Side): void {
  const entry = active[side];
  if (entry) entry.dismissed = true;
}

/** Test / shutdown helper: stop every active session. */
export function stopAllSmartWakeSessions(): void {
