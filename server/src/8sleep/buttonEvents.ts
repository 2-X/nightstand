...
interface Pending {
  ...
  down: boolean;
  // A hold-abort arrived for this button while down: the eventual release must
  // NOT emit a click.
  suppressed: boolean;
}
...
export class ButtonEventMachine {
   ...
   */
  public push(msg: string): ButtonEvent[] {
  ...
  private onPressLine(m: RegExpExecArray): ButtonEvent[] {
    const side: ButtonSide = m[1].toUpperCase() === 'L' ? 'left' : 'right';
    const edge = m[2].toLowerCase();
    const code = Number(m[3]);
    const button = CODE_TO_BUTTON[code];
    if (!button) return []; // unknown GPI code
    ...
    if (!state?.down) return []; // release without a tracked press (already consumed)
    if (state.suppressed) return []; // a hold-abort ate this click
    return [{ side, button, kind: 'click' }];
  }

  private onHeldLine(m: RegExpExecArray): ButtonEvent[] {
...
// Middle-button double-click (two `click`s within a window) maps to alarm
// dismiss. Kept separate from the edge machine so the window is easy to test
// and configure per side. Top/bottom single clicks pass straight through.

export interface DoubleClickResult {
  // Emit a single-click action for this event now (top/bottom always; middle
  // only if it turns out NOT to be the first half of a double-click - see note).
  singleClick?: ButtonEvent;
  // A middle double-click completed on this side.
  doubleClick?: { side: ButtonSide };
}
...
export class DoubleClickDetector {
  private lastMiddleClickAt: Partial<Record<ButtonSide, number>> = {};

  constructor(private windowMs: number) {}

  public setWindow(windowMs: number): void {
    this.windowMs = windowMs;
  }

  public feed(event: ButtonEvent, now: number): DoubleClickResult {
