...
const importsOf = (source) => (ts.preProcessFile(source, true, true).importedFiles.map((f) => f.fileName));
        ...
        if (specifier.startsWith(alias)) {
            ...
            if (!path.posix.extname(resolved) && !existsSync(path.join(repoRoot, resolved))) {
                for (const ext of ['.ts', '.tsx']) {
                    if (existsSync(path.join(repoRoot, resolved + ext)))
                        return resolved + ext;
                }
            }
            return resolved;
...
describe('the agent manifest', () => {
    ...
    it('never lists a stock-contract path as an agent file', () => {
        for (const contractPath of STOCK_CONTRACT.paths) {
            assert.ok(!agentPaths.has(contractPath), `${contractPath} is both an agent file and a stock dependency; it must be one or the other`);
        }
    });
});
    ...
    it('never reaches outside the manifest and the declared stock contract', () => {
        ...
        for (const entry of AGENT_MANIFEST) {
            ...
            for (const specifier of importsOf(source)) {
                ...
                if (agentPaths.has(resolved) || contractPaths.has(resolved))
                    continue;
...
//# sourceMappingURL=agentManifest.test.js.map