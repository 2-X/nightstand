...
export const SMART_WAKE_INTENSITY_STEPS = [20, 40, 60, 80];
export const SMART_WAKE_GAP_SECONDS_STEPS = [90, 60, 40, 25];
...
export class SmartWakeSession {
    ...
    async tick() {
        if (this.ended)
            return { depth: 'unknown', decision: 'done' };
        ...
        try {
            ...
            if (this.firstUnknownAtMs === null)
                this.firstUnknownAtMs = nowMs;
            return this.decideWithDepth('unknown', nowMs, null, null, null, median(this.movementSamples), this.hrBaseline);
        }
        ...
        const vitalsFresh = snap.biometricsHealthy &&
            freshVital !== null &&
        ...
        if (freshMove !== null)
            movementVal = freshMove.total_movement;
        const depth = this.estimateDepth(vitalsFresh, hr, movementVal, br, nowMs, priorMovementMedian, priorHrBaseline);
        const result = this.decideWithDepth(depth, nowMs, hr, movementVal, br, priorMovementMedian, priorHrBaseline);
        ...
        if (br !== null)
            this.brSamples.push(br);
        if (movementVal !== null)
            this.movementSamples.push(movementVal);
        return result;
    }
    // --- depth estimation ----------------------------------------------------
    estimateDepth(vitalsFresh, hr, movementVal, br, nowMs, priorMovementMedian, priorHrBaseline) {
        if (!vitalsFresh) {
            if (this.firstUnknownAtMs === null)
                this.firstUnknownAtMs = nowMs;
            return 'unknown';
        ...
        if (priorMovementMedian === null && priorHrBaseline === null)
            return 'deep';
        const movementElevated = movementVal !== null && priorMovementMedian !== null && priorMovementMedian > 0 &&
        ...
        const hrElevated = hr !== null && priorHrBaseline !== null &&
    ...
    // --- decision ------------------------------------------------------------
    decideWithDepth(depth, nowMs, hr, movementVal, br, priorMovementMedian, priorHrBaseline) {
        ...
        // depth estimation.
        const movementSpike = movementVal !== null && priorMovementMedian !== null && priorMovementMedian > 0 &&
            movementVal >= priorMovementMedian * SMART_WAKE_MOVEMENT_SPIKE_FACTOR;
        const hrRise = hr !== null && priorHrBaseline !== null &&
            hr >= priorHrBaseline + SMART_WAKE_HR_RISE_BPM;
        ...
        const unknownPastGrace = depth === 'unknown' &&
            this.firstUnknownAtMs !== null &&
            nowMs - this.firstUnknownAtMs >= SMART_WAKE_UNKNOWN_GRACE_MS;
        const shouldNudge = depth === 'light' || unknownPastGrace;
    ...
    windowQuarter(nowMs) {
        ...
        if (windowMs <= 0)
            return SMART_WAKE_INTENSITY_STEPS.length - 1;
    ...
    currentIntensity(nowMs) {
        return SMART_WAKE_INTENSITY_STEPS[this.windowQuarter(nowMs)];
    }
    currentGapSeconds(nowMs) {
        return SMART_WAKE_GAP_SECONDS_STEPS[this.windowQuarter(nowMs)];
    }
    // --- helpers -------------------------------------------------------------
    freshestVital(vitals) {
        let best = null;
        for (const v of vitals) {
            if (best === null || v.timestamp > best.timestamp)
                best = v;
        }
        return best;
    }
    freshestMovement(movement) {
        let best = null;
        for (const m of movement) {
            if (best === null || m.timestamp > best.timestamp)
                best = m;
        }
        return best;
    }
...
//# sourceMappingURL=smartWake.js.map