...
mock.module(new URL('./applyTemperatureChange.js', import.meta.url).href, {
    namedExports: {
        applyTemperatureDelta: async (side, current, delta) => {
...
mock.module(new URL('../routes/deviceStatus/updateDeviceStatus.js', import.meta.url).href, {
    namedExports: {
        updateDeviceStatus: async (status) => { updateCalls.push(status); },
    },
});
let execCalls = [];
mock.module(new URL('./deviceApi.js', import.meta.url).href, {
    namedExports: {
        executeFunction: async (command, arg = 'empty') => { execCalls.push({ command, arg }); },
        frankenCommands: {},
...
mock.module(new URL('../db/collector.js', import.meta.url).href, {
    namedExports: {
        recordEvent: (type, opts) => { recordedEvents.push({ type, opts }); },
        recordConfigAudit: () => { },
        startCollector: () => { },
    },
});
...
const logRec = (msg, seq = 1) => frameRecord(seq, cbor.encode({ type: 'log', ts: 1, level: 'info', msg, seq }));
function pressReleaseLog(sideTag, code, seq = 1) {
    return Buffer.concat([
...
function appendRaw(full, buf, mtimeSec) {
...
//# sourceMappingURL=buttonMonitor.test.js.map