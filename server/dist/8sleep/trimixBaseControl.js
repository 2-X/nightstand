...
function ticksToTorsoAngle(ticks) {
    return ticksToAngle(ticks, torsoAngleMap);
}
function ticksToLegAngle(ticks) {
...
function buildPacketWithChecksum(payload) {
...
function createSetAngleCommand(motor, angle, feedRate) {
    ...
    if (motor === 'torso') {
        ...
        if (ticks === undefined)
            throw new Error(`Invalid angle ${angle} for leg.`);
        view.setUint16(10, ticks, true);
    }
...
export class TriMixBaseControl {
    isConnected = false;
    notifyCharacteristicPath = null;
    writeCharacteristicPath = null;
    baseAddress = null;
    baseConfig = null;
    bleProcess = null;
    inGattMenu = false;
     ...
     */
    async initialize() {
        // Pods without an adjustable base have no config file at all - that's a
        // normal state, not an error. Log once and stay dormant instead of
    ...
    startBleProcess() {
        ...
        this.bleProcess.stdout?.on('data', (data) => {
            this.handleBleOutput(data.toString());
        });
        this.bleProcess.stderr?.on('data', (data) => {
    ...
    handleBleOutput(output) {
        ...
        if (this.isConnected && (output.includes('Disconnected') ||
    ...
    processNotificationBuffer() {
        ...
        while (this.notificationBuffer.length >= 20) {
            ...
            const startIndex = this.notificationBuffer.findIndex((_byte, i) => this.notificationBuffer[i] === 0xff &&
    ...
    parseNotification(packet) {
        ...
        if (packetType === 0x22) {
            ...
            const positionChanged = this.lastPosition.head !== headAngle ||
    ...
    async sendPayload(payload) {
        ...
        this.sendCommandToProcess(`select-attribute ${this.writeCharacteristicPath}`);
    ...
    async setPosition(position) {
        ...
        try {
            ...
            const torsoCommand = createSetAngleCommand('torso', torsoAngle, position.feedRate);
            await this.sendPayload(torsoCommand);
            await new Promise((resolve) => setTimeout(resolve, 500));
            const legCommand = createSetAngleCommand('leg', legAngle, position.feedRate);
    ...
    getClosestAngle(angle, map) {
        const angles = Array.from(map.keys()).sort((a, b) => a - b);
        return angles.reduce((prev, curr) => Math.abs(curr - angle) < Math.abs(prev - angle) ? curr : prev);
    }
    async stop() {
...
//# sourceMappingURL=trimixBaseControl.js.map