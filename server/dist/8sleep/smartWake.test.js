...
import { SmartWakeSession, SMART_WAKE_INTENSITY_STEPS, SMART_WAKE_GAP_SECONDS_STEPS, SMART_WAKE_UNKNOWN_GRACE_MS, } from './smartWake.js';
class Harness {
    nowMs;
    snapshot;
    dismissed = false;
...
function deepSnapshot(nowSec, hr = 50) {
    const vitals = [
...
function lightSnapshot(nowSec, hr, movement) {
    return {
...
function makeParams(deadlineMs, windowMinutes = 30) {
...
describe('SmartWakeSession', () => {
    ...
    it('escalates intensity and gap across the window quarters', async () => {
        ...
        assert.deepEqual(gotIntensities, [...SMART_WAKE_INTENSITY_STEPS], 'intensity should escalate 20->40->60->80 across quarters');
    });
    it('respects the per-quarter gap between pulses (UNKNOWN fallback)', async () => {
        ...
        const s = new SmartWakeSession(makeParams(deadline, windowMin), h.deps());
        await s.start();
        const dead = { vitals: [], movement: [], present: true, biometricsHealthy: false };
        // Anchor the unknown-grace window, then advance into the SECOND half of
        // the window (fallback nudging is deferred there by the passive-first-half
        // policy) and past the grace so nudging begins.
        h.snapshot = dead;
        await s.tick();
        h.nowMs = start + (windowMin / 2) * MIN + 30_000;
        h.snapshot = dead;
        let r = await s.tick();
        assert.equal(r.decision, 'pulse', 'first fallback nudge fires past the grace window');
        const q = Math.floor(((h.nowMs - start) / (windowMin * MIN)) * SMART_WAKE_GAP_SECONDS_STEPS.length);
        const gapNow = SMART_WAKE_GAP_SECONDS_STEPS[q];
        h.nowMs += 10_000; // inside the current quarter gap
        h.snapshot = dead;
        r = await s.tick();
        assert.equal(r.decision, 'wait', 'should wait inside the gap');
        h.nowMs += (gapNow * 1000); // now past the gap
        h.snapshot = dead;
        r = await s.tick();
    ...
    it('biometrics-dead: stays UNKNOWN then nudges after the grace window', async () => {
        ...
        // Past the grace but still in the passive first half: stay silent (we
        // cannot see natural arousals with dead biometrics, and the first half
        // protects REM-rich sleep).
        h.nowMs += SMART_WAKE_UNKNOWN_GRACE_MS + 30_000;
        h.snapshot = { vitals: [], movement: [], present: true, biometricsHealthy: false };
        r = await s.tick();
        assert.equal(r.decision, 'wait', 'passive first half defers the fallback');
        assert.equal(h.pulses.length, 0);
        // Second half: UNKNOWN past grace now nudges as a fallback.
        h.nowMs = start + 21 * MIN;
        h.snapshot = { vitals: [], movement: [], present: true, biometricsHealthy: false };
        r = await s.tick();
        assert.equal(r.depth, 'unknown');
    ...
    it('passive first half: LIGHT without a movement spike (REM-like) waits', async () => {
        const start = Date.parse('2026-06-01T06:00:00Z');
        const deadline = start + 30 * MIN;
        const h = new Harness(start);
        const s = new SmartWakeSession(makeParams(deadline, 30), h.deps());
        await s.start();
        ...
        // REM-like: HR elevated + variable breathing, but NO movement (atonia).
        const nowSec = Math.floor(h.nowMs / 1000);
        h.snapshot = {
            vitals: [
                { timestamp: nowSec - 20, heart_rate: 58, hrv: 55, breathing_rate: 19 },
            ],
            movement: [{ timestamp: nowSec - 40, total_movement: 5 }],
            present: true,
            biometricsHealthy: true,
        };
        const r = await s.tick();
        assert.notEqual(r.decision, 'pulse', 'REM-like light reading must not pulse in the first half');
        assert.equal(h.pulses.length, 0);
    });
    it('passive first half: a natural movement arousal DOES get ridden', async () => {
        const start = Date.parse('2026-06-01T06:00:00Z');
        const deadline = start + 30 * MIN;
        const h = new Harness(start);
        const s = new SmartWakeSession(makeParams(deadline, 30), h.deps());
        ...
        // Genuine arousal: movement spike + HR rise.
        h.snapshot = lightSnapshot(Math.floor(h.nowMs / 1000), 58, 40);
        const r = await s.tick();
        assert.equal(r.decision, 'pulse', 'a real arousal in the first half is ridden');
        assert.equal(h.pulses.length, 1);
    });
    it('commit-once: after the first pulse, a return to DEEP keeps escalating', async () => {
        const start = Date.parse('2026-06-01T06:00:00Z');
        const deadline = start + 30 * MIN;
        const h = new Harness(start);
        const s = new SmartWakeSession(makeParams(deadline, 30), h.deps());
        ...
        h.snapshot = lightSnapshot(Math.floor(h.nowMs / 1000), 58, 40);
        let r = await s.tick();
        assert.equal(r.decision, 'pulse', 'escalation starts on the arousal');
        // Depth flips back to deep-looking. Once committed, we keep going after
        // the gap rather than stranding the sleeper mid-wake.
        h.nowMs += 2 * MIN;
        h.snapshot = deepSnapshot(Math.floor(h.nowMs / 1000), 50);
        r = await s.tick();
        assert.equal(r.decision, 'pulse', 'committed escalation continues through a DEEP reading');
...
//# sourceMappingURL=smartWake.test.js.map