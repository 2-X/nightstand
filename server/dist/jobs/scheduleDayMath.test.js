...
const captured = [];
const scheduleJobMock = mock.fn((name, rule) => {
    captured.push({ name, rule: rule });
    return null;
});
...
const WEEK = 7 * 1440;
const abs = (dayIndex, hour, minute) => dayIndex * 1440 + hour * 60 + minute;
const ruleOf = (nameFragment) => {
...
const absOf = (r) => abs(r.dayOfWeek, r.hour, r.minute);
const minutesAfter = (a, b) => ((a - b) % WEEK + WEEK) % WEEK;
const pad = (n) => String(n).padStart(2, '0');
const describeRule = (r) => `${DAYS_OF_WEEK[r.dayOfWeek] ?? r.dayOfWeek} ${pad(r.hour)}:${pad(r.minute)}`;
const side = 'left';
beforeEach(() => {
    captured.length = 0;
});
describe('power on / power off land on a consistent pair of days', () => {
    ...
    cases.forEach(({ label, day, on, off }) => {
        it(`${label} (${on} -> ${off} on ${day}): off fires after on, within 24h`, () => {
            ...
            assert.ok(delta > 0 && delta <= 1440, `power on ${describeRule(onRule)} -> power off ${describeRule(offRule)} = ${(delta / 60).toFixed(1)}h of heating`);
        });
    });
});
describe('temperature adjustments land inside their own power window', () => {
    it('22:00 -> 13:00 with a 03:00 adjustment', () => {
        ...
        assert.ok(tempDelta > 0 && tempDelta <= offDelta && offDelta <= 1440, `power on ${describeRule(onRule)}, power off ${describeRule(offRule)}, temp adjust ${describeRule(tempRule)}`);
    });
    it('09:00 -> 11:00 morning nap with a 10:00 adjustment', () => {
        ...
        const tempRule = ruleOf('temperature-adjustment');
        const tempDelta = minutesAfter(absOf(tempRule), absOf(onRule));
        assert.ok(tempDelta > 0 && tempDelta <= 1440, `power on ${describeRule(onRule)}, temp adjust ${describeRule(tempRule)} (${(tempDelta / 60).toFixed(1)}h later)`);
    });
});
describe('alarms land inside their own power window', () => {
    it('ordinary night 21:00 -> 09:00 with a 06:30 alarm', () => {
        ...
        assert.ok(alarmDelta > 0 && alarmDelta <= offDelta, `power on ${describeRule(onRule)}, power off ${describeRule(offRule)}, alarm ${describeRule(alarmRule)}`);
    });
    it('sleep-in night 22:00 -> 13:00 with a 06:30 alarm', () => {
        ...
        const alarmRule = ruleOf('-alarm');
        const alarmDelta = minutesAfter(absOf(alarmRule), absOf(onRule));
        assert.ok(alarmDelta > 0 && alarmDelta <= 1440, `power on ${describeRule(onRule)}, alarm ${describeRule(alarmRule)} -- alarm is ${(alarmDelta / 60).toFixed(1)}h after the bed turns on`);
    });
    it('alarm day is derived from the alarm time, not from power.off', () => {
        ...
        assert.equal(alarmWithAfternoonOff, alarmWithMorningOff, `a 06:30 alarm moved from ${DAYS_OF_WEEK[alarmWithMorningOff]} to ${DAYS_OF_WEEK[alarmWithAfternoonOff]} `
...
describe('prime scheduler derives the reboot hour by subtracting an hour', () => {
    it('a 00:30 prime time still schedules a daily reboot (at 23:30 the day before)', () => {
        const s = settings({ primePodDaily: { enabled: true, time: '00:30' } });
        ...
        const rule = rebootJob.rule;
        assert.ok(rule.hour >= 0 && rule.hour <= 23, `daily reboot rule has hour=${rule.hour}; node-schedule rejects it (RecurrenceRule.isValid() === false) so the reboot never fires`);
        assert.equal(rule.hour, 23, 'an hour before 00:30 is 23:30');
    });
    it('a 14:00 prime time schedules the reboot at 13:00 (control)', () => {
        const s = settings({ primePodDaily: { enabled: true, time: '14:00' } });
        schedulePrimingRebootAndCalibration(s);
        const rebootJob = captured.find((c) => c.name.startsWith('daily-reboot'));
        assert.ok(rebootJob);
        assert.equal(rebootJob.rule.hour, 13);
    });
});
//# sourceMappingURL=scheduleDayMath.test.js.map