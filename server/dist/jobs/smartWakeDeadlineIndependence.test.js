...
mock.method(nodeSchedule, 'scheduleJob', (...args) => {
...
let scheduleRecurringAlarms;
let recurringAlarmsDB;
let settingsDB;
before(async () => {
...
describe('smart-wake deadline independence', () => {
    ...
    it('arms the deadline even when scheduling the smart-wake job throws synchronously', async () => {
        ...
        scheduleRecurringAlarms(settingsDB.data, 'left');
        assert.ok(nodeSchedule.scheduledJobs[`left-recurring-${alarmId}`], 'deadline alarm job missing after a synchronous smart-wake scheduling failure');
        assert.equal(nodeSchedule.scheduledJobs[`left-recurring-${alarmId}-smartwake`], undefined, 'the smart-wake job should NOT be armed (its scheduling threw)');
        breakSmartWakeScheduling = false;
    });
});
//# sourceMappingURL=smartWakeDeadlineIndependence.test.js.map