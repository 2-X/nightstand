...
function dayMatches(recurrence, weekday, dayStart) {
    switch (recurrence.kind) {
        ...
        case 'everyNDays': {
            ...
            const anchor = moment.tz(recurrence.anchorDate, 'YYYY-MM-DD', dayStart.tz());
            if (!anchor.isValid())
                return false;
            ...
            if (diffDays < 0)
                return false; // before the anchor: no occurrences
 ...
 */
export function expandAlarmOccurrences(alarm, timeZone, fromMs, toMs) {
    const out = [];
    if (!alarm.enabled)
        return out;
    if (toMs <= fromMs)
...
//# sourceMappingURL=recurrenceExpansion.js.map