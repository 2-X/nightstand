...
const eventQueue = [];
const auditQueue = [];
const queuedRows = () => bedQueue.length + hubQueue.length + eventQueue.length + auditQueue.length;
const lastBed = {};
let lastHub;
let lastTransition;
let flushTimer;
 ...
 */
export function recordConfigAudit(kind, source, snapshot) {
    try {
        auditQueue.push({
            timestamp: nowSec(),
            kind,
...
export function onDeviceStatus(status) {
    const ts = nowSec();
...
function deriveTransitionEvents(status, _ts) {
    ...
    lastTransition = next;
    if (!prev)
        return; // seed only, no event on first snapshot
...
export async function flush() {
    ...
    try {
        ...
        logger.warn(`[collector] flush dropped ${beds.length + hubs.length + events.length + audits.length} rows: ${errMsg(error)}`);
    }
}
...
export async function pruneOldRows() {
    ...
    try {
        ...
        logger.info(`[collector] retention pruned bed=${bed.count} hub=${hub.count} events=${events.count} audit=${audit.count}`);
    }
    catch (error) {
