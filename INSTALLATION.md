...
1. I would recommend setting up steps 18 and 19 below (SSH access for debugging, and the firewall rules that block WAN traffic)
...
Blocking WAN access does not cost you updates: when a newer build of this
fork is published, the app's Settings page shows an Update button, and the
updater opens internet access just long enough to download before blocking it
again.
...
- `fs-update` - Downloads and installs the latest build of this fork from GitHub, with automatic backup and rollback (same script the app's Update button runs)
